...
from app.schemas.auth import UserSignInParam
...
class SignIn:
    async def sign_in(
        ...
        access_token = helper.create_access_token(user_data=user_data)
        refresh_token = helper.create_refresh_token(user_data=user_data)
    ...
    async def authenticate_user(
        ...
        is_correct_password = helper.verify_password(
            password=password, hash_password=user.hash_password
        )
